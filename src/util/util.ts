...
export const getRootUnits = (organizationUnitIds: number[], organizationTree: any[]) => {
    let ids: number[] = []
...
export const timeSince = (createdAt: number): string => {
    return DateTime.fromMillis(+createdAt).setZone('utc').toRelative() || ''
}
...
export const formatDate = (date: number): string => {
    return DateTime.fromMillis(+date).setZone('utc').toFormat('DDD  HH:mm')
}
...
export const formatDateToDay = (date: number): string => {
    return DateTime.fromMillis(+date).setZone('utc').toFormat('dd MMM yyyy')
}
export const formatDateToInput = (date: number): string => {
    const newDate =  DateTime.fromMillis(+date).setZone('utc').toFormat('yyyy-MM-dd')
    console.log(newDate)
    return newDate;
}
export const formatDateToInputWithTime = (date: number): string => {
    return DateTime.fromMillis(+date).setZone('utc').toFormat("yyyy-MM-dd'T'HH:mm")
}
...
export const minutesToDuration = (totalMinutes: number): { hours: number; minutes: number } => {
...
export const summarizePartInventory = (part: Part) => {
    const inventories = part.inventories ?? []
    return inventories.reduce(
        (acc, inventory) => {
            acc.onHand += Number(inventory.onHand ?? 0)
            acc.onOrder += Number(inventory.onOrder ?? 0)
            acc.reorderQuantity += Number(inventory.reorderQuantity ?? 0)
            return acc
        },
        { onHand: 0, onOrder: 0, reorderQuantity: 0 }
    )
}

export const matchesPartStockStatus = (
    part: Part,
    ...
    const summary = summarizePartInventory(part)
    const reorderPoint = Number(part.reorderPoint ?? 0)

    switch (normalized) {
        case 'in stock':
            return summary.onHand > 0
        case 'low stock':
            return (
                summary.onHand > 0 &&
                (reorderPoint > 0 ? summary.onHand <= reorderPoint : false)
            )
        case 'out of stock':
            return summary.onHand <= 0
        default:
...
export const getPartStockStatus = (part: Part): 'In Stock' | 'Low Stock' | 'Out of Stock' => {
    const summary = summarizePartInventory(part)
    const reorderPoint = Number(part.reorderPoint ?? 0)
    ...
    if (reorderPoint > 0 && summary.onHand <= reorderPoint) {
        return 'Low Stock'
    }
...
export const matchesPartVendor = (part: Part, vendorName?: string | null) => {
    if (!vendorName) {
        return true
    }
    ...
    const vendorPricings = part.vendorPricings ?? []
    const vendorMatch = vendorPricings.some((pricing) => {
        const name = pricing.vendor?.name
    ...
    const purchaseOrderLines = part.purchaseOrderLines ?? []
    return purchaseOrderLines.some((line) => {
        const name = line.purchaseOrder?.vendor?.name
        return name?.trim().toLowerCase() === normalized
    })
}
...
export const parseCSVContentWithErrors = (
    ...
    recipients: Array<{ name: string; email: string; phone?: string; companyName?: string }>
    errors: string[]
} => {
    ...
    // Parse data rows
    const recipients: Array<{ name: string; email: string; phone?: string; companyName?: string }> =
        []
    const errors: string[] = []
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        try {
            ...
            const email = headerMap.email !== undefined ? values[headerMap.email]?.trim() : ''
            const phone = headerMap.phone !== undefined ? values[headerMap.phone]?.trim() : ''
            const companyName =
                headerMap.companyName !== undefined ? values[headerMap.companyName]?.trim() : ''
            ...
            errors.push(`Row ${i + 1}: Error parsing row - ${error.message}`)
        }
    }
...
export const dateInputToEpoch = (value: string | null | undefined): number | null => {
    if (!value) {
        return null
    }
